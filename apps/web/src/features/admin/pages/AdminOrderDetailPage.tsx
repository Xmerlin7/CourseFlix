...
// Read-only — see AdminOrdersService's docblock (backend) for why orders
// have no admin-mutable actions.
export function AdminOrderDetailPage() {
  ...
  return (
    <>
      <PageHeader title={`طلب ${data.studentName}`} description={formatDateTime(data.createdAt)}>
        <span className={`chip ${statusLabel.chip}`}>{statusLabel.label}</span>
        <span className={`chip ${paymentLabel.chip}`} style={{ marginInlineStart: 8 }}>
          {paymentLabel.label}
        </span>
      </PageHeader>

...
/**
 * Renders the seed's course handouts as real, readable A4 PDFs.
 *
...
export async function buildDocumentPdf(
  ...
  document.setCreator('CourseFlix seed');
  document.setProducer('CourseFlix');
  // Pinned rather than `new Date()`: the seed re-runs `buildDocumentPdf`
  // on every `npm run seed`, and document.seed.ts upserts by checksum of
  // the output bytes — a wall-clock timestamp would make every rerun
  // "changed" and pile up orphaned file rows/storage for no reason.

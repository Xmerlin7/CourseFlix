...
export class CreateVideoTranscripts1785000082000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    `);
    await queryRunner.query(`
      CREATE INDEX "idx_video_transcripts_course_id" ON "video_transcripts" ("course_id");
    `);
    await queryRunner.query(`
      CREATE INDEX "idx_video_transcripts_lesson_id" ON "video_transcripts" ("lesson_id");
    `);
    await queryRunner.query(`
      CREATE INDEX "idx_video_transcripts_section_id" ON "video_transcripts" ("section_id");
    `);
    ...
    await queryRunner.query(`
      CREATE INDEX "idx_video_chunks_transcript_id" ON "video_chunks" ("video_transcript_id");
    `);
    await queryRunner.query(`
      CREATE INDEX "idx_video_chunks_vector_id" ON "video_chunks" ("vector_id");
    `);
  }
